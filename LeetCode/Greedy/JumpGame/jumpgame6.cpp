...
public:
       ...
       //定义一个新vector，存储需要的steps
        ...
        while(!dp.empty()){
            ...
            for(int i=0;i<n;i++){
                ...
                int front=dp.front();
                dp.pop();
                if(int before=front-1;before>0) change_step_and_dp(dis,before,dp,step);
                if(int after=front+1;after<size) change_step_and_dp(dis,after,dp,step);
                else if(after=size-1) return dis-1;
                auto fds=map.equal_range(arr[front]); //return std::pair<iterator,iterator> start,end
                for(auto it=fds.first;it!=fds.second;it++){
                    if(it->second!=front) change_step_and_dp(dis,it->second,dp,step);
                }
            }   
        }
        return step[size-1];
    ...
    void remove_dup(vector<int>& arr){
        ...
        vector<int> tmp;
        for(int i=0;i<size;i++){
            while(i+1<size&&i+2<size&&arr[i]==arr[i+1]&&arr[i+1]==arr[i+2]){i++;tag=true;}
            tmp.emplace_back(arr[i]);
        }
        if(tag) arr=move(tmp);// arr=tmp;
    }
};